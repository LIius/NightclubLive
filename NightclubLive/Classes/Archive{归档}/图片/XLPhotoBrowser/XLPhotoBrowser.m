...
@interface XLPhotoBrowser () <XLZoomingScrollViewDelegate, XLPhotoBrowserDelegate , UIScrollViewDelegate>
...
@property (nonatomic, strong) XLPhotoImp *imp;
@end
...
+ (instancetype)showPhotoBrowserWithImages:(NSArray *)images currentImageIndex:(NSInteger)currentImageIndex
{
    ...
    XLPhotoImp *imp = [XLPhotoImp new];
    imp.imgs = images;
    browser.imp = imp;
    [browser setActionSheetWithTitle:nil delegate:imp cancelButtonTitle:@"取消" deleteButtonTitle:nil otherButtonTitles:@"保存", nil];
    [browser show];
    return browser;
}
