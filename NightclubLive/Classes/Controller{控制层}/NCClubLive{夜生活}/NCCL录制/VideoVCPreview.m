...
#import "NCLocalVideoCollectionViewCell.h"
...
- (void)viewDidLoad
{
    ...
    if ([CheckTools isPermissionsWithType:MicrophonePermissions])
    {
    ...
    {
        [[UIApplication sharedApplication] getVideoRelatedAttributesAssetBlock:^(NSArray *nameArr, NSArray *lengthArray, NSArray *imageArray) {
            
            self.nameArray = [[NSMutableArray alloc] initWithArray:nameArr];
            self.imageArray = [[NSMutableArray alloc] initWithArray:imageArray];
            self.lengthArray = [[NSMutableArray alloc] initWithArray:lengthArray];
            
            [self.collectionView reloadData];
        }];
        
        self.result = [[UIApplication sharedApplication]assetsFetchResults];
...
{
    NSString *str = NSStringFromClass([NCLocalVideoCollectionViewCell class]);
...
{
    NCLocalVideoCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"NCLocalVideoCollectionViewCell" forIndexPath:indexPath];
    if (!cell) {
        cell = [[NSBundle mainBundle]loadNibNamed:@"NCLocalVideoCollectionViewCell" owner:nil options:nil].lastObject;
    }
    
    UIImage *imageOrigin = self.imageArray[indexPath.row];
    cell.videoImageView.image = [imageOrigin imageSizeTransformToTargetView:cell.videoImageView];
    cell.videoLength.text = self.lengthArray[indexPath.row];
...
//            };
//
//            [self.paipaiUpload handelVideoURL:url sourceVC:self];
//        });
//
//    }];
