...
- (void)setupNoneView {
    ...
    noneImageView.userInteractionEnabled = YES;
    [noneImageView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(noneImageAction:)]];
    [self addSubview:noneImageView];
}

- (void)noneImageAction:(id)sender {
    if (_noneDataCallBack) {
        _noneDataCallBack();
    }
}
...
- (void)setupViews {
    ...
    if (!_pageControl) {
        _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, self.frame.size.height - 15, ITEM_WIDTH, 8)];
        if (self.isShowTitle) {
...
//    _pageControl.pageIndicatorTintColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"point"]];//[UIColor whiteColor];
//    _pageControl.currentPageIndicatorTintColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"pointSelect"]];//[UIColor redColor];
    [_pageControl setValue:[UIImage imageNamed:@"pointSelect"] forKeyPath:@"_currentPageImage"];
    [_pageControl setValue:[UIImage imageNamed:@"point"] forKeyPath:@"_pageImage"];
