...
- (void)fetchLocaton:(NIMKitLocationResult)result{
    
    ShareLocationViewController *slVC = [ShareLocationViewController initSBWithSBName:@"ShareLocationSB"];
    [[UIApplication sharedApplication].keyWindow.rootViewController presentViewController:[[UINavigationController alloc] initWithRootViewController:slVC] animated:YES completion:nil];
    
    slVC.calkBlock = ^(NSDictionary *locationDic){
        
        NSString *name = locationDic[@"name"];
        CGFloat  longitude =  [locationDic[@"lo"] floatValue];
        CGFloat  latitude  = [locationDic[@"la"] floatValue];
        
        result(latitude,longitude,name);
    };
}
