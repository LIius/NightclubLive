...
{
    [_toolBar.voiceBtn setImage:selected? KGetImage(@"icon_speeker") :[UIImage nim_imageInKit:@"icon_toolview_keyboard_normal"] forState:UIControlStateNormal];
    [_toolBar.voiceBtn setImage:selected? KGetImage(@"icon_speeker") : [UIImage nim_imageInKit:@"icon_toolview_keyboard_pressed"] forState:UIControlStateHighlighted];
}

- (void)updateEmotAndTextBtnImages:(BOOL)selected
{
    [_toolBar.emoticonBtn setImage:selected? KGetImage(@"icon_face") :[UIImage nim_imageInKit:@"icon_toolview_keyboard_normal"] forState:UIControlStateNormal];
    [_toolBar.emoticonBtn setImage:selected? KGetImage(@"icon_face"):[UIImage nim_imageInKit:@"icon_toolview_keyboard_pressed"] forState:UIControlStateHighlighted];
}
...
- (void)onTouchMoreBtn:(id)sender {
    if (_inputType != InputTypeMedia) {
        ...
        _inputBottomViewHeight = [[[NIMKitUIConfig sharedConfig] globalConfig] moreViewHeight];
        if ([self.toolBar.inputTextView isFirstResponder]) {
            [self.toolBar.inputTextView resignFirstResponder];
        }
        [UIView animateWithDuration:0.25 animations:^{
