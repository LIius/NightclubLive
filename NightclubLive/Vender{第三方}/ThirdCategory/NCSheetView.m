...
- (instancetype)initWithTitle:(NSString *)title items:(NSArray *)items
{
    ...
    if ( self )
    {
        ...
        NCSheetViewConfig *config = [NCSheetViewConfig globalConfig];
        ...
        [self mas_updateConstraints:^(MASConstraintMaker *make) {
            make.bottom.equalTo(self.cancelButton.mas_bottom).offset(8);
        }];
...
@interface NCSheetViewConfig()

@end

@implementation NCSheetViewConfig

+ (NCSheetViewConfig *)globalConfig
{
    static NCSheetViewConfig *config;
    static dispatch_once_t onceToken;
    
    dispatch_once(&onceToken, ^{
        
        config = [NCSheetViewConfig new];
