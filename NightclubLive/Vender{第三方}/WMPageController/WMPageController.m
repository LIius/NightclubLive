...
- (UIView *)bottomLine {
    if (!_bottomLine) {
        ...
        _bottomLine = [[UIView alloc] initWithFrame:CGRectMake(0,_menuViewFrame.size.height+20 + h, self.view.frame.size.width, self.bottomLineHeight ? self.bottomLineHeight : 2.0)];
        _bottomLine.backgroundColor = self.bottomLineColor ? self.bottomLineColor : [UIColor lightGrayColor];
    }
    return _bottomLine;
}

- (void)drawVerticalLineView{
    if (self.automaticallyCalculatesItemWidths) {
        NSLog(@"%@",self.menuView);
        return;
    }else{
        for (int i = 0; i<self.childControllersCount-1; i++) {
            CGFloat lineH = _menuViewFrame.size.height-self.verticalLineOffY*2;
            UIView *line = [[UIView alloc] initWithFrame:CGRectMake(self.view.frame.size.width/self.childControllersCount *(i+1) , _menuViewFrame.origin.y + self.verticalLineOffY, self.verticalLineWidth ? self.verticalLineWidth : 1, lineH==0 ? 1:lineH)];
            line.backgroundColor = self.verticalLineColor ? self.verticalLineColor : self.bottomLineColor;
...
- (void)wm_calculateSize {
    ...
    if ([self.dataSource respondsToSelector:@selector(pageController:preferredFrameForContentView:)]) {
        CGRect rect = [self.dataSource pageController:self preferredFrameForContentView:self.scrollView];
        _contentViewFrame = rect;
    } else {
...
- (void)wm_addMenuView {
    ...
    menuView.scrollViewColor = self.menuScrollviewColor;
    ...
    if (self.menuViewStyle == WMMenuViewStyleVerticalLineBottomLine) {
        [self drawVerticalLineView];
        [self.view addSubview:self.bottomLine];
    }
    if (self.menuViewStyle == WMMenuViewStyleVerticalLine ) {
        [self drawVerticalLineView];
    }
...
- (void)wm_adjustMenuViewFrame {
    ...
    if (self.menuCornerRadius > 0.0) {
        self.menuView.layer.cornerRadius = self.menuCornerRadius;
